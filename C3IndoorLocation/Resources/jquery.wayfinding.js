...
//							portal.idB = innerSegment.id;
		...
		function setAdsPoint(point, point2,el) {
			var start,
				attachPinLocation,
				x,
				y,
			...
			if (options.showLocation) {
				...
				var here=$('#circle', el);
				var start = $('#Doors #' + escapeSelector(options.startpoint), el);
				var x = (Number(start.attr('x1')) + Number(start.attr('x2'))) / 2;
				var	y = (Number(start.attr('y1')) + Number(start.attr('y2'))) / 2;
				
				
				here.attr('cx',x+point);
				here.attr('cy',y+point2);
			
			}
			
		} 
		
		function setCurrentPoint(point, point2,el) {
		...
		function activateSVG(el, svgDiv) {
			...
			$('#Coupons image', svgDiv).click(function (event) {
				
			});
			
			$('#beacons circle', svgDiv).click(function (event) {
				//console.log("hehe");
			});


			// Disable clicking on every SVG element except rooms
			$(svgDiv).find('*').css('pointer-events', 'auto');
			$('#Rooms a', svgDiv).find('*').css('pointer-events', 'auto');
			$('#Coupons image', svgDiv).find('*').css('pointer-events', 'auto');
			$('#beacons circle', svgDiv).find('*').css('pointer-events', 'auto');
			$(el).append(svgDiv);
		...
		function rattleimage(el,svgDiv){ 
			if (shakecount==100) 
			{
				return;
			}
				...
				if(shake==1)
				{
					$('#Coupons image', svgDiv).animate({opacity:'1'},"slow");	
					
				}
				else
				{
					$('#Coupons image', svgDiv).animate({opacity:'0.5'},"slow");	
												
				}
				shake=1-shake;
				shakecount++;
				setTimeout("rattleimage(el,svgDiv)",1000);
		} 
			...
			if (action && typeof (action) === 'string') {
				...
				case 'adspoint':
					...
					if (passed === undefined) {
						...
						setAdsPoint(passed, passed2,obj);					
					}
				...
				case 'currentpoint':
					...
					if (passed === undefined) {
						...
						setCurrentPoint(passed, passed2,obj);					
					}
